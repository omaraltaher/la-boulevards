...
- **Phase 2 complete:**
  ...
  - ✅ Multi-venue events — 36 LA venues in `venue-events.json` (Ticketmaster + Eventbrite, daily). Theatre popup shows upcoming shows. Boulevard info panel lists shows under each theatre name. The Wiltern appears on both Wilshire and Western. Eventbrite uses `/organizers/:id/` and `/venues/:id/` endpoints (search API deprecated 2020).
- **Architecture:** `makeClusterSystem(cfg)` factory replaces 4 duplicate cluster systems (parrots/coyotes/wells/lions). `_openWildlifePopup` shared panel-click handler. `_idx` pre-indexing on wildlife items. DOM nodes cached at init. `isMobile()` cached with resize listener. Info panel section order: cameras → parrots → lions → coyotes → theatres → art → wells.
- **GitHub Actions (12 workflows):** Daily: cameras (6am PT), wiltern (7am PT), venues (8am PT). Monthly 1st: lions, coyotes, theatres, parrots, art, wells. Yearly Jan 1: railway, faults, tacos. All support `workflow_dispatch`.
